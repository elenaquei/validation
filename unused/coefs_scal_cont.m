classdef coefs_scal_cont % coefficients of the scalar equations - continuation case
    ...
    methods
        % COEFS
        function alpha=coefs_scalar(Sscal,Svec,Dscal,Dvec,Non0,Pscal,Pvec,val,xdot)
            ...
            if nargin ==8
                ...
                if nargin==9
                    if min(size(xdot))~=1
                        error('X_dot must be a vector');
                    elseif lenght(xdot)< Sscal+Svec
                        error('x_dot must be at least as long as N+M')
                    end
                    x_dot = xdot;
                end
            end
