...
% gives a numerical reason to start the saddle check

if ~numerical_check
    saddle_confirmed = 0;
    return
end
% which variables should be checked by the saddle
...
% refinement in smaller intervals for validation
for i = 1: n_intervals
    ...
    %figure(30); hold on; plot(i,x_prime0_int,'*') % plots the first derivative
    %title('first der numerics')
    %figure(40); hold on; plot(i,x_prime_prime0_int,'*')% plots the second derivative
    %title('second der numerics')
    if ~numerical_check
        continue
    elseif talkative>1
        disp('Trying to validate a sub segment')
    end
    ...
    %DEBUG
%     figure(100); hold on; plot(i,saddle_x0_int.scalar(x0.size_scalar+ num_variable),'*') % plots the first derivative
%     title('first der Newton VS num')
%     figure(200); hold on; plot(i,saddle_x0_int.scalar(2*x0.size_scalar+ num_variable),'*')% plots the second derivative
%     title('Second der Newton VS num')
%     figure(100); hold on; plot(i,x_prime0_int,'d') % plots the first derivative
%     figure(200); hold on; plot(i,x_prime_prime0_int,'d')% plots the second derivative
    
    %figure(10); hold on; plot(i+1,saddle_x1_int.scalar(x0.size_scalar+ num_variable),'*') % plots the first derivative
    %figure(20); hold on; plot(i+1,saddle_x1_int.scalar(2*x0.size_scalar+ num_variable),'*')% plots the second derivative
    
    %figure(10); hold on; plot(i+1,x_prime1_int,'d') % plots the first derivative
    %figure(20); hold on; plot(i+1,x_prime_prime1_int,'d')% plots the second derivative
    %n_intervals=i+10;
    %continue
    %debug
    %if i==24
