...
global use_intlab 
global talkative
temp_talkative = talkative;
temp_use_intlab = use_intlab;
use_intlab = 0;

if nargin<5 || ~exist('num_variable','var')
    num_variable = 1:x0.size_scalr;
end

% check for the first derivative to pass zero numerically
[numerical_check] = if_saddle_numerical(F_old,F_new, x0,x1,num_variable);
% if it doesn't, return
if ~numerical_check
    saddle_confirmed = 0;
    return
elseif talkative>0
    disp('Numerical hint of the existence of a saddle node will be further researched')
end
...
% split into a lot (n_intervals) of smaller intervals and test numerically
% in there
x_old = x0;
for i = 1: n_intervals
    [x0_int, x1_int, F0_int, F1_int] = automatic_refinement(F_old, F_new, x0, x1, n_intervals, i); % done
    %if talkative>2
    %    disp('Refinement in the segment done')
    %end
    
    [numerical_check, x_prime0_int, x_prime1_int,x_prime_prime0_int,...
        x_prime_prime1_int] = if_saddle_numerical(F0_int, F1_int, x0_int, x1_int,num_variable);
%         
%     % % DEBUGGING
%     lambda0(i,:) = [x0_int.scalar(2), x_prime0_int(2),x_prime_prime0_int(2)];
%     T(i,:) = [x0_int.scalar(1), x_prime0_int(1),x_prime_prime0_int(1)];
%     amplitude(i,:) = [x0_int.scalar(3), x_prime0_int(3),x_prime_prime0_int(3)];
%     
...
%     % % END DEBUGGING
    ...
    if ~numerical_check % if numerically no saddle, continue the search
        continue
    end
    x1_int_new = Newton_2(x1_int, F1_int);
    x_old = x1_int;
    % if numerically there is a saddle in the sub-segment, try validating
    % it
    if talkative >1
        disp('Further hint of a saddle node in refined segment')
    end
    % expand the system into the bigger validation system
    [saddle_F0_int, saddle_F1_int] = bigger_saddle_system(F0_int, F1_int, x0_int, x1_int);
    % compute numerically the solutions at the two endpoints
    saddle_x0_int = compute_saddle(saddle_F0_int,x0_int,x_prime0_int,x_prime_prime0_int);
    saddle_x1_int = compute_saddle(saddle_F1_int,x1_int,x_prime1_int,x_prime_prime1_int);
    % check if the numerical solutions fit in the saddle requests
    [bool_saddle, z_crosses]=check_if_saddle_possible(saddle_x0_int,saddle_x1_int);
    
    if bool_saddle && z_crosses
        error('what to do now?')
    end
    % if they do, keep on validating
    if bool_saddle
        if talkative>1
            disp('Numerics shows a saddle node')
        end
        % need more refinements? (some smart way of determining this)
        [delta, bool_saddle_confirmed] = bound_refinement(saddle_x0_int, saddle_x1_int, saddle_F0_int, saddle_F1_int); % done
        if bool_saddle_confirmed
            saddle_confirmed = bool_saddle_confirmed;
            use_intlab = temp_use_intlab; %RMK: not using intlab in here, problem
            return
        end
        n_intervals_ref = ceil(1/delta)+1;
        % refine the segment again
        for j = 1: n_intervals_ref
            ...
            % computing numerical solutions at the endpoints of the
            % interval 
...
F0_int = F_old;
F1_int = F_old;
for j =1:3 % loop on the components of linear_coef
    lin_coef_step_temp = (1/n_intervals)*(F_new.scalar_equations.linear_coef{j} - F_old.scalar_equations.linear_coef{j});
    ...
    F0_int.scalar_equations.linear_coef{j} = F_old.scalar_equations.linear_coef{j} + (i-1) *lin_coef_step_temp;
    ...
    F1_int.scalar_equations.linear_coef{j} = F_old.scalar_equations.linear_coef{j} + i *lin_coef_step_temp;
...
% function saddle_x = compute_saddle(saddle_F,x,y,z)
%
% inputs: big system and 3 small solutions
% output: big solution
y = vec2Xi_vec(y,x);
z = vec2Xi_vec(z,x);
...
% second derivative is crossing too the zero axes
...
function [delta, bool_saddle_confirmed] = bound_refinement(saddle_x0, saddle_x1, saddle_F0, saddle_F1)
...
z_bound = min(min(abs(z0)),min(abs(z1)));
...
y0 = midrad(y0_point(num_variable),Imin_sad);
y1 = midrad(y1_point(num_variable),Imin_sad);

z0 = midrad(z0_point(num_variable),Imin_sad);
z1 = midrad(z1_point(num_variable),Imin_sad);

% check if validation confirmed saddle
if any(abs(z0)<Imin_sad) || ...
        any(abs(z1)<Imin_sad) || any(abs(y0)<Imin_sad) || ...
        any(abs(y1)<Imin_sad)
    error('Bounds too big!')
end
