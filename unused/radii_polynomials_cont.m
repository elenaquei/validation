function [flag,Imin,Imax,new_iter,Yvector,Z0vector,Z1vector,Z2vector,new_step] = radii_polynomials_cont(xBar0,xBar1,DH0,DH1,...
    alpha,coefs_linear0, coefs_linear1,previous_iter,A0,A1)
% INPUT:
% xBar0      Xi_vector, approximate solution of the problem in x_0;
% xBar1      Xi_vector, approximate solution of the problem in x_1;
% alpha      coefs, coefficients of the ODE equations;
% coefs_linear   2-cell with the coefficients of the linear part of the
% system;
...
if xBar0.size_scalar~=xBar1.size_scalar || xBar0.size_scalar ~= alpha.size_scalar
    error('Scalar dimensions are not consistent')
end
if xBar0.size_vector~=xBar1.size_vector || xBar0.size_vector ~= alpha.size_vector
...
if use_intlab
    ...
    for i=1:alpha.size_vector
        alpha.value{i}=intval(alpha.value{i});
    end
    %for i=1:2
...
try
    if ~isempty(previous_iter.Y)
        [Yvector,new_iter_Y,Ys]=Y_bound_cont(A0,A1,xBar0,xBar1,alpha,coefs_linear0,coefs_linear1,previous_iter.Y);
    else
        [Yvector,new_iter_Y,Ys]=Y_bound_cont(A0,A1,xBar0,xBar1,alpha,coefs_linear0,coefs_linear1);
    end
    
    if talkative>1
        fprintf('Computed Y, time %s\n',datestr(now,13));
    end
catch
    return
end
...
try
    if ~isempty(previous_iter.Z1)
        [Z1vector,new_iter_Z1,Z1s]=Z1_bound_cont(A0,A1,xBar0,xBar1,alpha,...
            coefs_linear0,coefs_linear1,Adagger_delta,previous_iter.Z1);
    else
        [Z1vector,new_iter_Z1,Z1s]=Z1_bound_cont(A0,A1,xBar0,xBar1,alpha,...
            coefs_linear0,coefs_linear1,Adagger_delta);
    end
    if talkative>1
        fprintf('Computed Z2, time %s\n',datestr(now,13));
    end
catch
    return
end

[Z2vector,Z2s]=Z2_bound_cont(A0,A1,xBar0,xBar1,alpha);
...
try
    ...
    if talkative>1
        fprintf('Computed validation interval, time %s\n\n',datestr(now,13));
    end
catch
    return
end
