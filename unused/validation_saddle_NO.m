...
% backup made the 3rd of July, before messing with the rescaling of the
...
for i = 1: n_intervals
    ...
    derivative_saddle = 0;
    tries=0;
    while derivative_saddle<10^-6 && tries<20
        tries=tries+1;
        Rescaling=Rescaling*5
        [saddle_F0_int, saddle_F1_int] = bigger_saddle_system(F0_int, F1_int, x0_int, x1_int,Rescaling); 
        ...
        [index_saddle]=determine_index(saddle_x0_int,saddle_x1_int);
        y0_point = (saddle_x0_int.scalar(size_scalar+1:size_scalar*2));
        y1_point = (saddle_x1_int.scalar(size_scalar+1:size_scalar*2));
        derivative_saddle = abs(y0_point(index_saddle)- y1_point(index_saddle));
    end
    if tries==20
        error('Refinement by rescaling did not work out')
    end
    ...
    if bool_saddle
        ...
        bool_saddle_tot = saddle_validation(saddle_x0_int, saddle_x1_int, saddle_F0_int, saddle_F1_int,num_variable);
...
function [index_saddle]=determine_index(x0,x1)
% function [bool_saddle, z_crosses]=check_if_saddle_possible(x0_int,x1_int)
%
% inputs: 2 big solutions
% output: index of the possible saddle node, just checking hte first
% derivative
...
[index_saddle]=determine_index(x0,x1);
