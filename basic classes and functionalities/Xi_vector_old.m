classdef Xi_vector
    ...
    methods
        ...
        %
        %         % SET.IFFT
        %         function x=set.ifft_vector(x,vec_ifft)
        %
        %             warning('Not a good idea, please use set_ifft instead');
        %
        %             if x.size_vector~= 0 && any(size(vec_ifft)~=size(x.vector))
        %                 if any(size(vec_ifft.')~=size(x.vector))
        %                     error('sizes input not compatible');
        %                 else
        %                     vec_ifft=vec_ifft.';
        %                 end
        %             end
        %             x.bool_ifft=1;
        %             x.ifft_vector = vec_ifft;
        %
        %         end
        %         % end SET.IFFT
        ...
        %                 xscal=abs(x.scalar);
        %                 yscal=abs(y.scalar);
        %                 z.scalar=max(xscal.sup,yscal.sup);
        %                 xvec=abs(y.vector);
        %                 yvec=abs(x.vector);
        %                 z.vector=max(xvec.sup,yvec.sup);
        %             else
        %                 z.scalar=max(abs(x.scalar),abs(y.scalar));
        %                 z.vector=max(abs(y.vector),abs(x.vector));
        %             end
        %         end
        %         % MAX
