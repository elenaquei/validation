...
A_sim(all_scal, all_scal) = (A_sim(all_scal, all_scal) + conj(A_sim(all_scal, all_scal)))/2;

for j=1:x.size_vector
    for i = 1:length(all_scal)
        vec=x.size_scalar+(j-1)*length_nodes+1:x.size_scalar+j*length_nodes;
        A_sim(vec,i)=sym_vec(A_sim(vec,i));
        A_sim(i,vec)=sym_vec(A_sim(i,vec));
    end
end
